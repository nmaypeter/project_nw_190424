                        ...
                        for wpiwp in wpiwp_seq:
                            ...
                            for sample_count in range(sample_number):
                                ...
                                while temp_sequence[0] <= total_budget and len(celf_sequence) != 0:
                                    ...
                                    print('@ mpmis seed selection @ data_set_name = ' + dataset_name + '_' + cascade_model + ', dist = ' + str(wallet_distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    while cur_budget < begin_budget and mep_i_node != '-1':
                                        ...
                                        if round(cur_budget + sc, 2) >= begin_budget and mep_flag == seed_set_length:
                                ...
                                while len(s_matrix_sequence) != total_budget:
                                    s_matrix_sequence.append(0)
                                    c_matrix_sequence.append(0)
                                    ss_k_time_sequence.append(ss_k_time_sequence[-1])

                                for begin_budget in range(1, total_budget + 1):
                                    ss_strat_time = time.time()
                                    s_matrix = s_matrix_sequence[begin_budget - 1]
                                    c_matrix = c_matrix_sequence[begin_budget - 1]
                                ...
                                for ppp in ppp_seq:
                                    ...
                                    for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                        if sample_seed_set != 0:
                                            print('@ mpmis evaluation @ dataset_name = ' + dataset_name + '_' + cascade_model + ', dist = ' + wallet_distribution_type + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    path1 = 'result/mpmis_' + wallet_distribution_type + '_ppp' + str(ppp) + '_wpiwp' * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = 'result/mpmis_' + wallet_distribution_type + '_ppp' + str(ppp) + '_wpiwp' * wpiwp + '/' + dataset_name + '_' + cascade_model + '_' + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                    fw.write('pmis, ppp = ' + str(ppp) + ', total_budget = ' + str(bud) + ', dist = ' + wallet_distribution_type + ', wpiwp = ' + str(wpiwp) + '\n' +
