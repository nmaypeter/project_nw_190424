                        ...
                        for wpiwp in wpiwp_seq:
                            ...
                            for sample_count in range(sample_number):
                                ...
                                while len(temp_sequence) != 0:
                                    ...
                                    print('@ mhed seed selection @ dataset_name = ' + dataset_name + '_' + cascade_model + ', dist = ' + str(wallet_distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                ...
                                for ppp in ppp_seq:
                                    ...
                                    for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                        if sample_seed_set != 0:
                                            print('@ mhed evaluation @ dataset_name = ' + dataset_name + '_' + cascade_model + ', dist = ' + wallet_distribution_type + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    path1 = 'result/mhed_' + wallet_distribution_type + '_ppp' + str(ppp) + '_wpiwp' * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = 'result/mhed_' + wallet_distribution_type + '_ppp' + str(ppp) + '_wpiwp' * wpiwp + '/' + dataset_name + '_' + cascade_model + '_' + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                    fw.write('hed, ppp = ' + str(ppp) + ', total_budget = ' + str(bud) + ', dist = ' + wallet_distribution_type + ', wpiwp = ' + str(wpiwp) + '\n' +
