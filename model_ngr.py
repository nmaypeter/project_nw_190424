                    ...
                    for wallet_distribution in wallet_distribution_seq:
                        ...
                        for wpiwp in wpiwp_seq:
                            ...
                            for sample_count in range(sample_number):
                                ...
                                celf_sequence = ssng_main.generateCelfSequenceR()
                                ...
                                while len(temp_sequence) != 0:
                                    ...
                                    print('@ mngr seed selection @ dataset_name = ' + dataset_name + '_' + cascade_model + ', dist = ' + str(wallet_distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    while now_budget < begin_budget and mep_i_node != '-1':
                                        ...
                                        if mep_flag == seed_set_length:
                                            ...
                                            if mg_ratio_g > 0:
                                                celf_item_g = (mep_k_prod, mep_i_node, mg_ratio_g, flag_g)
                                ...
                                for ppp in ppp_seq:
                                    ...
                                    for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                        if sample_seed_set != 0:
                                            print('@ mngr evaluation @ dataset_name = ' + dataset_name + '_' + cascade_model + ', dist = ' + wallet_distribution_type + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    path1 = 'result/mngr_' + wallet_distribution_type + '_ppp' + str(ppp) + '_wpiwp' * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = 'result/mngr_' + wallet_distribution_type + '_ppp' + str(ppp) + '_wpiwp' * wpiwp + '/' + dataset_name + '_' + cascade_model + '_' + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                    fw.write('ngr, ppp = ' + str(ppp) + ', total_budget = ' + str(bud) + ', dist = ' + wallet_distribution_type + ', wpiwp = ' + str(wpiwp) + '\n' +
